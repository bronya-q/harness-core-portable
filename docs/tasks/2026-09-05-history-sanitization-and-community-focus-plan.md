...
> **执行更新（2026-09-05 后）**：仓库所有者已明确授权，已完成三轮 mirror 演练与 `git filter-repo` 过滤，并 force-update 远端 main 与 tag。三轮范围依次为：私人人格标识、本机路径类（Windows 用户目录绝对路径、微信数据目录、`~/Documents` 下的私有工作区、桌面路径、模型目录及变体）、本机私有知识库路径与旧本机文件名路径。过滤后全部可达 refs 的上述命中为 0。上文“不批准远端重写”为当时状态。
