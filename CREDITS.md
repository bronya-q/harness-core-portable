...
- 仅称“用户自己的数据”仍不自动保证其中所有第三方消息都有公开再分发权。
...
### 4.7 本机私人人格与知识库实验（内部 provenance）

- 来源性质：用户本机存在若干私人人格与知识库实验，用于验证本仓库的通用角色、知识域、审查与工程职责抽象。
- 与本仓库的关系：这些实验提供了**一般性设计启发**，没有随仓库分发其人格正文、私人记忆、日记、信件、语料、图片、语音或本地数据库。
- 设计影响（概括性）：

  | 借鉴方向 | 本仓库对应 |
  |---|---|
  | 认知动力、长短目标、主动候选 | `cognitive_dynamics.py`、`mind_evolution.py`、`proactive_pipeline.py` |
  | 日记、信件与长期沉淀 | `mind_precipitate.py`、humanization diary/letter 功能 |
  | 人格维持、知识库/卡片式组织 | `p4_experiment.py`、`natural_session.py`、`perspective_card.py`、`user_model_signals.py` |
  | 独立 persona/source/scope 路由 | `runtime_resolver.py` 的通用注册表抽象 |

- 未随仓库分发：私人人格正文、私人记忆、日记、信件、原始语料和本地数据库。
- 权利边界：任何本地材料（包含原著、译文、注释、数据库）的公开许可不能仅因本地存在而推定。
- 当前结论：可以披露为本地内部设计来源，不能推定所有源材料均可按 MIT 发布；具体私人名称与内容不进入公开默认包。
