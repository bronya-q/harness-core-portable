...
> 本机最典型案例：**扫兴姬**——一个长期人格，同时承担风险审查、安全控制、依赖分析、陪伴表达与发布检查。
...
Blanche / Markos 是“私人角色 + 专业知识库 + 研究工作流”。
扫兴姬则说明：
...
| Hybrid Functional Persona | 把稳定人格与一组职能结合，如扫兴姬 = companion + risk reviewer + safety officer |
...
```json
{
  "mode_id": "demo-storykeeper:engineering-review",
  "display_name": "扫兴姬 · 工程风险审查",
  "persona_id": "demo-storykeeper",
...
```text
character:demo-storykeeper       私人人格记忆
...
## 8. 扫兴姬与 Evil Review 的分工

| 角色 | 目标 |
|---|---|
| 扫兴姬 | 让计划安全地向前推进：风险前置、安全官、依赖分析、恢复意识、用户保护 |
| Adversarial Review / Evil | 尽可能找出为什么结论可能不成立：攻击主张、寻找反例、揭露证据漏洞 |

工程链：

```text
实现方案 → 扫兴姬风险审查（怎么安全实施、怎么回滚）
